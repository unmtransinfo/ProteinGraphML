...
def generateTrainPredictFromExcel(inFile, idType, negProtein=None):
    ...
    df = pd.read_excel(
        inFile, sheet_name="Sheet1"
    )  # change 'Sheet1' to the name in your spreadsheet
    if idType == "symbol":
        symbols = df["Symbol"].values.tolist()
        symbolLabel = df.set_index("Symbol").T.to_dict("records")[
            0
        ...
        proteinIdLabel = df.set_index("Protein_id").T.to_dict("records")[
            0
        ]  # DataFrame to dictionary
        # Protein_Ids for training set
...
def generateTrainPredictFromRDS(inFile, negProtein=None):
    ...
    df = rdsdata[None]
    trainData[True] = set(
        df.loc[(df["Y"] == "pos") & (df["subset"] == "train")]["id1"]
    )
    trainData[False] = set(
        df.loc[(df["Y"] == "neg") & (df["subset"] == "train")]["id1"]
    )
...
def saveTrainPredictSet(trainData, predictData, outDir, outBaseName):
    ...
    with open(pklTrainFile, "wb") as handle:
        logging.info(
            "Training dataset: {0} positive, {1} negative, {2} total".format(
                len(trainData[True]),
                len(trainData[False]),
                len(trainData[True]) + len(trainData[False]),
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate training and predict set definition files.",

...
def saveTrainPredictSet(trainData, predictData, outDir, outBaseName):
    ...
    with open(pklTrainFile, 'wb') as handle:
        logging.info("Training dataset: {0} positive, {1} negative, {2} total".format(len(trainData[True]),
                                                                                      len(trainData[False]),
                                                                                      len(trainData[True]) + len(
                                                                                          trainData[False])))
...
if __name__ == '__main__':
    ...
    if fileExt.lower() == 'rds':
        ...
        trainData, predictData = generateTrainPredictFromRDS(args.ifile, negProtein=(
            negProteinIds if args.use_default_negatives else None))
    elif fileExt.lower() in ('xlsx', 'xls'):
        logging.info('Input file with ID type "{0}" specified: {1}'.format(args.symbol_or_pid, fileName))
        trainData, predictData = generateTrainPredictFromExcel(args.ifile, args.symbol_or_pid, negProtein=(
            negProteinIds if args.use_default_negatives else None))
    elif fileExt.lower() in ('csv', 'tsv', 'txt'):
        logging.info('Input file with ID type "{0}" specified: {1}'.format(args.symbol_or_pid, fileName))
        trainData, predictData = generateTrainPredictFromText(args.ifile, args.symbol_or_pid, negProtein=(
            negProteinIds if args.use_default_negatives else None))
    else:
        pass  # ERROR

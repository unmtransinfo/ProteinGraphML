...
parser.add_argument(
    ...
    type=str,
    nargs="?",
    help="Mammalian Phenotype ID, e.g. MP_0000180",
)
parser.add_argument(
    "--file",
    type=str,
    nargs="?",
    help='input file, pickled training set, e.g. "diabetes.pkl"',
)
parser.add_argument(
    "--dir",
    default=DATA_DIR,
    help='input dir (default: "{0}")'.format(DATA_DIR),
)
parser.add_argument(
    "--resultdir",
    type=str,
...
parser.add_argument(
    "--crossval_folds",
    type=int,
    default=NUM_OF_FOLDS,
    help='number of folds for average CV (default: "{0}")'.format(
        NUM_OF_FOLDS
    ),
)
parser.add_argument(
...
logging.basicConfig(
    ...
    level=(logging.DEBUG if argData["verbose"] > 1 else logging.INFO),
)
...
if disease is None and fileName is None:  # NO INPUT
    ...
    try:
        ...
        logging.error(
            "Must generate pickled training set file for the given disease"
        )
        exit()
...
if fileData is not None:
    ...
    trainData = metapathFeatures(
        ...
        idDescription,
        staticFeatures,
        loadedLists=fileData,
    ).fillna(0)
else:
    trainData = metapathFeatures(
        disease, currentGraph, nodes, idDescription, staticFeatures
    ).fillna(0)

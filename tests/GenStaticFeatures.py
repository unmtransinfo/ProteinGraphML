...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--db", choices=DBS, default="tcrd", help="({0})".format("|".join(DBS))
    )
    parser.add_argument("--outputdir", default=".")
    parser.add_argument(
        "--sources",
        help="comma-separated list: {0}".format(",".join(SOURCES)),
    ...
    if len(set(sources) - set(SOURCES)) > 0:
        parser.error(
            "Invalid sources: {0}".format(
                ",".join(list(set(sources) - set(SOURCES)))
            )
        )
    ...
    if "gtex" in sources:
        ...
        logging.info(
            "GTEX: rows: {0}; cols: {1}".format(gtex.shape[0], gtex.shape[1])
        )
    ...
    if "hpa" in sources:
        ...
        logging.info(
            "HPA: rows: {0}; cols: {1}".format(hpa.shape[0], hpa.shape[1])
        )
    ...
    if "lincs" in sources:
        ...
        logging.info(
            "LINCS: rows: {0}; cols: {1}".format(
                lincs.shape[0], lincs.shape[1]
            )
        )
    ...
    if "ccle" in sources:
        try:
            ...
            logging.info(
                "CCLE: rows: {0}; cols: {1}".format(
                    ccle.shape[0], ccle.shape[1]
                )
            )
            ccle.round(args.decimals).to_csv(ofile_ccle, "\t", index=True)
        except Exception as e:
            logging.error(
                "Failed to generate static features for CCLE: {0}".format(e)
            )
        logging.info(
            "{0}: elapsed time: {1}".format(
                os.path.basename(sys.argv[0]),

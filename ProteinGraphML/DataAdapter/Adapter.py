...
class OlegDB(Adapter):
    ...
    # static features
    def loadGTEX(self):
        gtex = selectAsDF("SELECT protein_id, median_tpm, tissue_type_detail FROM gtex",
                          ["protein_id", "median_tpm", "tissue_type_detail"], self.db)
        logging.debug("(OlegDB.loadGTEX) GTEx rows returned: {0}".format(gtex.shape[0]))
        return gtex

    def loadCCLE(self):
        ccle = selectAsDF("SELECT protein_id, cell_id, tissue, expression FROM ccle",
                          ["protein_id", "cell_id", "tissue", "expression"], self.db)
        logging.debug("(OlegDB.loadCCLE) CCLE rows returned: {0}".format(ccle.shape[0]))
        return ccle

    def loadLINCS(self):
        lincs = selectAsDF("SELECT protein_id, pert_id||':'||cell_id AS col_id, zscore FROM lincs",
                           ["protein_id", "col_id", "zscore"], self.db)
    ...
    def load(self):
        ...
        mousePhenotype = selectAsDF("select * from mousephenotype",
                                    ["protein_id", "mp_term_id", "p_value", "effect_size", "procedure_name",
                                     "parameter_name", "association"], db)
        ...
        combinedSet = attachColumn(mouseToHumanMap, mousePhenotype,
                                   "protein_id")  # just bind the protein ID from our last table
        ...
        parentHierarchy = GraphEdge("mp_term_id", "parent_id", edge=None,
    ...
    def fetchPathwayIdDescription(self):
        ...
        kegg = selectAsDF("select distinct kegg_pathway_id, kegg_pathway_name from kegg_pathway",
...
"""
        ...
        logging.debug(
            "(OlegDB.fetchNegativeClassProteinIds) Negative class Protein Ids: {0}".format(negProteinIds.shape[0]))
...
class TCRD(Adapter):
    ...
    def loadPFAM(self, proteinFilter=None):
        pfam = selectAsDF("SELECT DISTINCT protein_id, value AS entry_ac FROM xref WHERE xtype = 'Pfam'",
                          ["protein_id", "entry_ac"], self.db)
        if proteinFilter is not None:
    ...
    def loadPROSITE(self, proteinFilter=None):
        prosite = selectAsDF("SELECT DISTINCT protein_id, value AS entry_ac FROM xref WHERE xtype = 'PROSITE'",
                             ["protein_id", "entry_ac"], self.db)
        if proteinFilter is not None:
...
"""
        omim = selectAsDF(sql,
                          ['protein_id', 'symbol', 'phenotype_id', 'clinical_significance', 'phenotype.name', 'mim_id',
                           'mim_title'], self.db)
    ...
    def loadLINCS(self):
        lincs = selectAsDF("SELECT protein_id, CONCAT(pert_dcid, ':', cellid) AS col_id, zscore FROM lincs",
                           ["protein_id", "col_id", "zscore"], self.db)
    ...
    def load(self):
        ...
        logging.debug("(TCRD.load) humanProteinList.homologene_group_id.nunique(): %d" % (
        ...
        logging.debug(
            "(TCRD.load) mouseProteinList.protein_id_m.nunique(): %d" % (mouseProteinList.protein_id_m.nunique()))
        logging.debug("(TCRD.load) mouseProteinList.homologene_group_id.nunique(): %d" % (
        ...
        mpOnto = selectAsDF("SELECT mpid AS mp_term_id, parent_id, name FROM mpo", ["mp_term_id", "parent_id", "name"],
                            db)
    ...
    def fetchPathwayIdDescription(self):
        ...
        ppi = selectAsDF("SELECT DISTINCT id AS protein_id, description AS name FROM protein", ["protein_id", "name"],
                         self.db)
    ...
    def fetchSymbolForProteinId(self):
        proteinIdSymbol = selectAsDF("SELECT DISTINCT id AS protein_id, sym AS symbol FROM protein",
                                     ['protein_id', 'symbol'], self.db)
    ...
    def fetchUniprotForProteinId(self):
        proteinIdUniprot = selectAsDF("SELECT DISTINCT id AS protein_id, uniprot AS uniprot_id FROM protein",
                                      ['protein_id', 'uniprot_id'], self.db)
        logging.debug("(TCRD.fetchUniprotForProteinId) proteinIdUniprot rows: {0}".format(proteinIdUniprot.shape[0]))
        proteinIdUniprotDict = proteinIdUniprot.set_index('protein_id').T.to_dict('records')[
            0]  # DataFrame to dictionary
...
"""
        negProteinIds = selectAsDF(sql, ['protein_id'], self.db)
        logging.debug(
            "(TCRD.fetchNegativeClassProteinIds) Negative class Protein Ids: {0}".format(negProteinIds.shape[0]))
        return negProteinIds

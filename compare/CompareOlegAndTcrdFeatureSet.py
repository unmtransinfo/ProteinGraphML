...
def check_feature_difference(cols, odata, tdata, dp):
    ...
    for col in cols:
        if isinstance(col, int):
            r_col_data = set([round(float(v), dp) for v in tdata['pp.' + str(col)]])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--olegfile', required=True, help='Features pickle file generated using OlegDB')
    parser.add_argument('--tcrdfile', required=True, help='Features pickle file generated using TCRD')
    ...
    if selected_cols == 0:
        ...
        matched_cols, unmatched_cols = check_feature_difference(selected_cols, olegdata, tcrddata, args.decimalplace)
        print("Number of features with the same values in OlegDB and TCRD files...{0}".format(len(matched_cols)))
        print("Number of features with the different values in OlegDB and TCRD files...{0}".format(len(unmatched_cols)))

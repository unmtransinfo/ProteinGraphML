...
def draw_bar_plot(xdataTr, xdataPr, train, predict, imgfile):
    ...
    ax.set_xlabel('Top N')
    ax.set_title('\n'.join(wrap('Common protein Ids in classifications using TCRD and OlegDB (Autophagy)', 60)))
    ax.set_xticks(xPr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tcrdTr', required=True, help='Classification results file for training data using '
                                                              'TCRD database')
    parser.add_argument('--tcrdPr', required=True, help='Classification results file for prediction data using '
                                                              'TCRD database')
    parser.add_argument('--olegTr', required=True, help='Classification results file for training data using '
                                                              'OlegDB database')
    parser.add_argument('--olegPr', required=True, help='R classification results file for prediction data '
    ...
    n = round(len(TcrdListTr), -2) + 200 if len(TcrdListTr) > round(len(TcrdListTr), -2) else round(
        len(TcrdListTr), -2) + 100
    if n>1000:
        n = 1000

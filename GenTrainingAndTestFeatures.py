...
def saveTrainPredictSet(
    allData, outputDir, disease=None, trainingfile=None, predictfile=None
):
    """
    ...
    # print (PredictData)
        ...
        pklPredictFile = (
            outputDir
            + "/"
            + os.path.basename(predictfile).split(".")[0]
            + "_PredictData.pkl"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate features for training and predict set",
    ...
    parser.add_argument(
        "--trainingfile", help='pickled training set, e.g. "diabetes.pkl"'
    )
    parser.add_argument(
        "--predictfile",
        help='pickled predict set, e.g. "diabetes_predict.pkl"',
    )
    parser.add_argument(
        "--outputdir",
        required=True,
        ...
        if args.predictfile is not None:
            ...
            try:
                with open(args.predictfile, "rb") as f:
                    fileData.update(
                        pickle.load(f)
                    )  # fileData will now have both train and predict set
            except:
